...
var DocumentTransform = class _DocumentTransform {
   ...
   */
  static identity() {
    return new _DocumentTransform(identity, {
      cache: false
    });
  }
  /**
  ...
  updateQuery(options, update) {
    return this.batch({
      update(cache) {
        ...
        cache.writeQuery(__spreadProps(__spreadValues({}, options), {
          data
        }));
        return data;
      }
  ...
  updateFragment(options, update) {
    return this.batch({
      update(cache) {
        ...
        const data = update(value);
        if (data === void 0 || data === null) return value;
        cache.writeFragment(__spreadProps(__spreadValues({}, options), {
          data
        }));
        return data;
      }
...
var mergeTrueFn = (existing, incoming, {
  mergeObjects
}) => mergeObjects(existing, incoming);
var mergeFalseFn = (_, incoming) => incoming;
var defaultStreamFieldMergeFn = (existing, incoming, {
  streamFieldInfo,
  existingData
}) => {
...
function makeMergeFieldFunctionOptions(policies, objectOrReference, fieldSpec, context, storage, existingData) {
  ...
  if (extensions && streamInfoSymbol in extensions) {
    const _b = extensions, {
      [_a = streamInfoSymbol]: streamInfo
    } = _b, otherExtensions = __objRest(_b, [
      __restKey(_a)
    ]);
...
function warnAboutDataLoss(existingRef, incomingObj, storeFieldName, store) {
  ...
  __DEV__ && invariant.warn(112, fieldName, parentType, childTypenames.length ? "either ensure all objects of type " + childTypenames.join(" and ") + " have an ID or a custom merge function, or " : "", typeDotName, Array.isArray(existing) ? [...existing] : __spreadValues({}, existing), Array.isArray(incoming) ? [...incoming] : __spreadValues({}, incoming));
}
...
var ObservableQuery = class {
  ...
  */
  ...
  fetchMore({
    ...
    const combinedOptions = __spreadProps(__spreadValues({}, compact(this.options, {
      errorPolicy: "none"
    }, {
      query,
      context,
  ...
  */
  ...
  fetch(options, networkStatus, fetchQuery, operator) {
    ...
    const subscription = observable.pipe(tap({
      ...
      next: (value) => {
        ...
        this.input.next(__spreadProps(__spreadValues({}, value), {
          query,
          variables,
          meta
        }));
  ...
  maskResult(result) {
    ...
    return masked === result.data ? result : __spreadProps(__spreadValues({}, result), {
      data: masked
    });
  }
  dirty = false;
  ...
  */
  ...
  operator = filterMap((notification) => {
    ...
    if (notification.source === "cache") {
      ...
      result = notification.kind === "E" ? __spreadProps(__spreadValues({}, isEqualQuery(previous, notification) ? previous.result : {
        data: void 0,
        dataState: "empty",
        partial: true
      }), {
  ...
  startGraphQLSubscription(options) {
    ...
    const observable = (this.getDocumentInfo(query).hasClientExports ? from(this.localState.getExportedVariables({
      ...
      const {
        ...
        restart: res
      } = this.getObservableFromLink(query, context, variables2, fetchPolicy, extensions);
      const queryInfo = new QueryInfo(this);
      restart = res;
...
function removeStreamDetailsFromExtensions(original) {
  ...
  const _a = original, {
    ...
    [_c = streamInfoSymbol]: _
  } = _d, extensions = __objRest(_d, [
    __restKey(_c)
  ]), result = __objRest(_a, [
    "extensions"
  ]);
...
var ApolloClient = class {
  ...
  readQuery(options, optimistic = false) {
    return this.cache.readQuery(__spreadProps(__spreadValues({}, options), {
  ...
  readFragment(options, optimistic = false) {
    return this.cache.readFragment(__spreadProps(__spreadValues({}, options), {
  ...
  constructor(options = {}) {
    ...
    super((operation) => {
      ...
      const {
        options: options2,
        body
      } = selectHttpOptionsAndBodyInternal(operation, print3, fallbackHttpConfig, linkConfig, contextConfig);
...
//# sourceMappingURL=chunk-DFMRRW2W.js.map
